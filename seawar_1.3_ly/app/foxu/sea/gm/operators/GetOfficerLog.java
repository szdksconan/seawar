...
public class GetOfficerLog extends GMOperator
{
		...
		JSONArray jsonArray,ServerInfo info)
	{
		...
		try
		{
			...
			OfficerTrack[] tracks=null;
			if("2".equals(type))
				tracks=objectFactory.getOfficerTrackMemCache().loadTracks(
					player.getId(),sTime,eTime,OfficerTrack.ADD);
			else if("3".equals(type))
				tracks=objectFactory.getOfficerTrackMemCache().loadTracks(
					player.getId(),sTime,eTime,OfficerTrack.REDUCE);
			else
				tracks=objectFactory.getOfficerTrackMemCache().loadTracks(
			...
			if(tracks!=null)
			{
				...
				{
					json=new JSONObject();
					json.put("sid",tracks[i].getOfficerSid());
