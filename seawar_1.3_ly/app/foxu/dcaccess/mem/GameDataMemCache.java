...
/**
 * 内存管理 这个不用写redis 数据库每12个小时保存
 */
public class GameDataMemCache extends MemCache
{
...
//	public static final int FORCE_DB_TIME=5;
	...
	public void init()
	{
		...
		/** 当日运营数据 */
		String sql="SELECT * FROM game_data where plat is null and the_year="
			+year+" and the_month="+month+" and the_day="+day;
		// 数据库加载所有邮件数据
		GameData gameData[]=(GameData[])dbaccess.loadBySql(sql);
		if(gameData!=null)
		{
			for(int i=0,n=gameData.length;i<n;i++)
			{
				GameDataSave gameDataSave=new GameDataSave();
				gameDataSave.setData(gameData[i]);
				// 设置保存的时间
				gameDataSave.setSaveTimeDB(TimeKit.getSecondTime());
				cacheMap.put(gameData[i].getId(),gameDataSave);
			}
		}
		// 判断数据库最后一条记录是不是今天
		...
		{
			GameDataSave gameDataSave=(GameDataSave)object[i];
			GameData data=gameDataSave.getData();
			if(data.getThe_year()==year&&data.getThe_month()==month
				&&data.getThe_day()==day)
			{
				save=gameDataSave;
				break;
		...
		if(save==null)
		{
			...
			data.setThe_day(day);
			gameDataSave.setData(data);
			cacheMap.put(data.getId(),gameDataSave);
			save=gameDataSave;
		}
	...
	{
		String sql_order="SELECT plat_id from orders GROUP BY plat_id";
		...
		Fields[] fields=orderAccess.loadSqls(sql_order);
		if(fields!=null)
		{
			for(int i=0;i<fields.length;i++)
			{
				String key=((StringField)fields[i].get("plat_id")).value;
				if(key==null) continue;
				keys.add(key);
			}
		}
		fields=orderAccess.loadSqls(sql_user);
		if(fields!=null)
		{
			for(int i=0;i<fields.length;i++)
			{
	...
	public int getTotalUser(String plat)
	{
		...
		if(plat!=null) sql+=" where plat='"+plat+"'";
	...
	public int someDayAllDau(int day,String plat)
	{
		...
		String sql="SELECT count(*) FROM users WHERE login_time>="
			+someDayBegin;
		if(plat!=null) sql+=" and plat='"+plat+"'";
		return SeaBackKit.loadBySqlOneData(sql,factory.getUserDBAccess());
	}
	...
	public int getChargeUserDay(int day,String plat)
	{
		...
		int todayEndTime=SeaBackKit.getSomedayEnd(SeaBackKit.DAY_MILL_TIMES
			*day);
		String sql="SELECT count(distinct user_id) FROM orders WHERE create_at>="
	...
	/** 某天的拉新用户 */
	public int getSomeDayNewIntiver(int day)
	{
		// 某天的开始时间
		int someDayBegin=SeaBackKit
			.getSomedayBegin(SeaBackKit.DAY_MILL_TIMES*day);
		...
		int someDayEnd=SeaBackKit.getSomedayEnd(SeaBackKit.DAY_MILL_TIMES
			*day);
		String sql="SELECT count(*) FROM players WHERE create_at>="
			+someDayBegin+" AND create_at<="+someDayEnd+" AND inveted>0";
		return SeaBackKit.loadBySqlOneData(sql,factory.getUserDBAccess());
	}
	...
	public int getChargeUsers(String plat)
	{
		...
		if(plat!=null) sql+=" where plat_id='"+plat+"'";
		return SeaBackKit.loadBySqlOneData(sql,
			factory.getOrderCache().dbaccess);
	}
	...
	/**
	 * 更新数据 bool=true为强制获取最新数据 并更新数据库 强制获取最新数据的时间间隔也要1个小时
	 */
	public long updata(boolean bool,int forceTime)
	{
		...
		if(day!=data.getThe_day())
		{
			...
			cacheMap.put(data.getId(),gameDataSave);
			save=gameDataSave;
			return (TimeKit.getMillisTime()-time);
		}
		// 强制获取最新数据 任然需要1个小时的间隔
		if(bool)
		{
			if((save.getForceDbTime()+forceTime)>nowTime) return 0;
		...
		if((save.getSaveTimeDB()+GAME_DATA_DB_TIME)>nowTime) return 0;
		long time=TimeKit.getMillisTime();
		// 更新最新数据
		flushTodayData(data,0);
		// 存储数据
	...
	public void flushTodayData(GameData data,int offset)
	{
		...
		int someDayNewUsers=getSomeDayNewUsers(1+offset,data.getPlat());
		int last_day_rate=0;
		// 隔日重登率 扩大10000倍
		...
		someDayNewUsers=getSomeDayNewUsers(3+offset,data.getPlat());
		...
		int week_rate=0;
		someDayNewUsers=getSomeDayNewUsers(7+offset,data.getPlat());
		// 大于7天重登率 扩大10000倍
		...
		someDayNewUsers=getSomeDayNewUsers(14+offset,data.getPlat());
		...
		// 大于30天重登率 扩大10000倍
		int month_rate=0;
		someDayNewUsers=getSomeDayNewUsers(30+offset,data.getPlat());
		...
		someDayNewUsers=getSomeDayNewUsers(60+offset,data.getPlat());
		...
		// 新增 扩大10000倍
		int arpu1=0;
		someDayNewUsers=getSomeDayNewUsers(1,data.getPlat());
		...
		int arpu3=0;
		someDayNewUsers=getSomeDayNewUsers(3,data.getPlat());
		...
		int arpu7=0;
		someDayNewUsers=getSomeDayNewUsers(7,data.getPlat());
		...
		int arpu14=0;
		someDayNewUsers=getSomeDayNewUsers(14,data.getPlat());
		...
		int arpu30=0;
		someDayNewUsers=getSomeDayNewUsers(30,data.getPlat());
		...
		int arpu60=0;
		someDayNewUsers=getSomeDayNewUsers(60,data.getPlat());
		...
		Calculation(data);
		if(data.getPlat()==null) data.setOnlineTime(cpuOnlineTime(dau));
	}
	 ...
	 */
	public void setSave(GameDataSave save)
	{
		this.save=save;
	}
	...
	public int getSomeCharge(int day,int offset,String plat)
	{
		...
		String sql="select count(orders.money) from orders,players where players.create_at>="
			+registerBegin
			+" and players.create_at<="
			+registerEnd
			+" and orders.create_at>="
			+registerBegin
			+" and orders.create_at<="
			+someDayEnd
			+" and orders.user_id=players.id";
		if(plat!=null) sql+=" and plat_id='"+plat+"'";
		float usmoney=0;
		// 6元
	...
	public synchronized void incrOnline(String plat)
	{
		if(plat==null) return;
		Integer num=onlineMap.get(plat);
		if(num==null) num=0;
		onlineMap.put(plat,num++);
		GameDataSave save=saveMap.get(plat);
		if(save!=null&&num>save.getMaxOnLine())
		{
			save.setMaxOnLine(num);
		}
	}

	public synchronized void decrOnine(String plat)
	{
		if(plat==null) return;
		Integer num=onlineMap.get(plat);
	...
	public int getOnline(String plat)
	{
	...
	{
		save.getData().setLoginCount(save.getData().getLoginCount()+1);
	}
